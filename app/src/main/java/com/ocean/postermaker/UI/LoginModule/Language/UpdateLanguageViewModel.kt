...
class UpdateLanguageViewModel (
    private val context: Context,
    private val apiService: ApiService,
    private val dataManager: DataManager) : BaseViewModel() {

        internal var mData: MutableLiveData<UpdateLanguageModel> = MutableLiveData()
        var requestCallLabel: Job? = null

        fun callLangUpdateApi(selectedLanguageId: String) {
            ...
            if (isInternetAvailable(context)) {
                requestCallLabel = CoroutineScope(Dispatchers.IO).launch {
                    val requestApi = apiService.UpdateUserLanguage(DataManager(context).getToken(),selectedLanguageId)
