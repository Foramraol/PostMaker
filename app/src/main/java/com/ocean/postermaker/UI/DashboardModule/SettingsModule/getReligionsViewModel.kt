...
class getReligionsViewModel (
    private val context: Context,
    private val apiService: ApiService,
    private val dataManager: DataManager) : BaseViewModel() {

        internal var mData: MutableLiveData<getReligionsModel> = MutableLiveData()
        var requestCallLabel: Job? = null

        fun callgetPRofileViewModel() {
            ...
            if (isInternetAvailable(context)) {
                requestCallLabel = CoroutineScope(Dispatchers.IO).launch {
                    val requestApi = apiService.getReligions(DataManager(context).getToken())
