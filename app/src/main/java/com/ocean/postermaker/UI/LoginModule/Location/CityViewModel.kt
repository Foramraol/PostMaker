...
class CityViewModel (
    ...
    private val dataManager: DataManager) : BaseViewModel() {
        ...
        internal var mData: MutableLiveData<CityModel> = MutableLiveData()
        var requestCallLabel: Job? = null

        fun callCityListApi(SelectedStateId: String) {
            ...
            if (isInternetAvailable(context)) {
                requestCallLabel = CoroutineScope(Dispatchers.IO).launch {
                    val requestApi = apiService.getCitiesList(DataManager(context).getToken(),SelectedStateId)
