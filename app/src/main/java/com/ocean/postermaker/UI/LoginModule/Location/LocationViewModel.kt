...
class UpdateLocationViewModel(
    ...
    internal var mData: MutableLiveData<UpdateLocationModel> = MutableLiveData()
    var requestCallLabel: Job? = null

    fun callUpdateLocationApi(
        ...
        if (isInternetAvailable(context)) {
            requestCallLabel = CoroutineScope(Dispatchers.IO).launch {
                val requestApi = apiService.UpdateUserLocation(
                    DataManager(context).getToken(),
                    SelectedCountry,
                    SelectedState,
                    SelectedCity
                )
